...
## Combining Equivalence Partitioning and Boundary Testing

**Equivalence Partitioning comes before Boundary Testing.** If your inputs have distinct behavioral ranges, define the classes first. You can't test boundaries until you know where the classes are.

1. **Identify equivalence classes** from requirements/code
2. **Pick boundaries** between classes
3. **Test boundaries** — the "just inside" values serve as interior representatives

**Don't add separate interior tests.** When you test "just above min boundary" (e.g., 9 chars), that value already represents the class interior. Adding another "interior" test (e.g., 12 chars) is redundant.
...
| PW-4 | Above min boundary | 9 chars | valid (interior representative) |
| PW-5 | Below max boundary | 19 chars | valid |
| PW-6 | At max boundary | 20 chars | valid |
| PW-7 | Above max boundary | 21 chars | too long |
```
